...
export default function MatchList() {
  ...
  // Добавить функцию обработки чекбоксов
  ...
  const filters = {
    leagues: selectedLeagues, one_o: selectedOneOs, one_e: selectedOneEs, 
    x_o: selectedXOs, x_e: selectedXEs, two_o: selectedTwoOs, two_e: selectedTwoEs,
    bts_o: selectedBtsOs, bts_e: selectedBtsEs, bts_no_o: selectedBtsNoOs,
    bts_no_e: selectedBtsNoEs, over_o: selectedOverOs, over_e: selectedOverEs,
    under_o: selectedUnderOs, under_e: selectedUnderEs, first_half: selectedFirstHalfs,
    match: selectedMatches, bts_result: selectedBtsResult, total_goals: selectedTotalGoals,
    team: selectedTeam, home: showHome, away: showAway
  };  
    ...
    return (
      <div style={{ ...filterItemStyle, ...(filterType === 'leagues' && { minWidth: '250px' }) }} data-attribute={dataAttribute}>
        ...
        <div style={{ position: 'relative' }}>
          <button onClick={() => setShowDropdown(!showDropdown)} style={selectWithDropdownStyle} title={getTitle()}>
            {getDisplayText()}
          </button>
          {showDropdown && (
        ...
        if (isCacheFresh()) {
          ...
          if (cached) {
            ...
            if (Array.isArray(parsed)) {
              setAllMatchesCache(parsed); // ← устанавливаем прямо в allMatchesCache
            }
          }
        } else {
  ...
  return (
    <div style={{ padding: '10px' }}>
      <div ref={useRef<HTMLDivElement>(null)} style={topBlockStyle}>
        {statistics && (
